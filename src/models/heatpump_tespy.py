...
"""
from tespy.components import CycleCloser, Compressor, Valve, HeatExchanger, Source, Sink, Condenser, Pump
...
class Heatpump_tespy():
    ...
    def setup_heat_pump(self):
        ...
        self.nwk = Network(fluids=[self.working_fluid, "air", "Water","INCOMP::MEG[0.2]|mass"], iterinfo=False)
        ...
        self.so1 = Source("ambient air source")
        self.si1 = Sink("ambient air sink")
        self.so2 = Source("heating source")
        self.si2 = Sink("heating sink")

        self.c0 = Connection(self.va, "out1", self.cc, "in1", label="0")
        self.c1 = Connection(self.cc, "out1", self.ev, "in2", label="1")
        self.c2 = Connection(self.ev, "out2", self.cp, "in1", label="2")
        ...
        self.c2.set_attr(T=self.tamb_design - self.ttd_heat_exchanger)
        ...
        self.cp.set_attr(eta_s=self.eta_compressor)
        ...
        self.c2.set_attr(fluid={self.working_fluid: 1, "Water": 0, "air": 0,"INCOMP::MEG[0.2]|mass": 0}, x=1.0)
        print("Tamb design value (as stored):", self.tamb_design)
        self.c10.set_attr(fluid={self.working_fluid: 0, "Water": 0, "air": 0,"INCOMP::MEG[0.2]|mass": 1},  T=self.tamb_design,p=1)
        self.c12.set_attr(p=None)
        self.c12.set_attr(T=Ref(self.c10, 1, -2))
        self.c21.set_attr(fluid={self.working_fluid: 0, "Water": 1, "air": 0,"INCOMP::MEG[0.2]|mass": 0}, p=3.0, T=self.heating_system_return_temp)
