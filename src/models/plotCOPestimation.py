...
df1_10 = df1.iloc[::10]
df2 = pd.read_csv('default_charline_simulation_results.csv',sep=',')
...
R2 = r2_score(df2['cop_given'], df2['cop'])
print("R^2 =", R2)

#Determine standard deviation sigma
error_abs = np.abs(df2['cop'].values - df2['cop_given'].values)
...
ax.plot(df2['cop_given'], df2['cop'], 'o',label = f'$R^2$ = {R2:.3f}, $\sigma$ = {std_error:.3f}')
ax.plot(x,y)
